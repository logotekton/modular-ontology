# Modular Ontology Completion Audit
...
| Ontology ZIP discovery | `modular_ontology/pack_index.py::discover_pack_files`, `list_packs` | `tests/test_modular_ontology.py::test_discovers_existing_revit_and_advance_steel_packs` | Implemented |
...
| Persistent server index | `modular_ontology/store.py`, SQLite `packs/documents/nodes/edges` | `test_sqlite_index_persists_pack_documents_and_graph` | Implemented |
...
| Admin-only pack permissions | `modular_ontology/auth.py`, upload/reindex authorization checks | `test_auth_sessions_distinguish_admin_and_member`, upload/reindex RBAC tests, browser admin/member UI smoke | Implemented for local RBAC |
| Configurable employee users | `MODULAR_ONTOLOGY_USERS_FILE`, `data/users.example.json` | `test_auth_can_load_users_from_json_config` | Implemented |
...
| MCP tools for GPT/Codex | `modular_ontology/mcp_server.py`, `mcp-config.example.json` | `scripts/verify_mcp_stdio.py`, `test_mcp_stdio_server_lists_and_calls_tools` | Implemented |
| Natural-language model query | `modular_ontology/qa.py`, `/api/query`, MCP `ask_pack_question` | `test_local_graph_rag_answer_uses_evidence_and_graph_context`, browser query smoke | Implemented |
| Optional OpenAI synthesis | `qa.py::use_openai`, env `MODULAR_ONTOLOGY_OPENAI_MODEL` | `test_openai_graph_rag_path_uses_injected_client` | Implemented with fallback |
...
- OpenAI synthesis requires user-provided `OPENAI_API_KEY` and `MODULAR_ONTOLOGY_OPENAI_MODEL`.
