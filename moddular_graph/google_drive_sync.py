...
from .config import DATA_DIR, DB_PATH, MCP_TOKENS_FILE
...
class GoogleDriveClient:
    ...
    def from_env(cls) -> "GoogleDriveClient":
        access_token = os.environ.get("MODDULAR_GRAPH_GOOGLE_DRIVE_ACCESS_TOKEN") or os.environ.get(
            "GOOGLE_DRIVE_ACCESS_TOKEN"
        )
        if access_token:
            return cls(access_token=access_token)

        if os.environ.get("MODDULAR_GRAPH_GOOGLE_DRIVE_USE_GCLOUD_AUTH") == "1":
        ...
        api_key = os.environ.get("MODDULAR_GRAPH_GOOGLE_DRIVE_API_KEY") or os.environ.get("GOOGLE_DRIVE_API_KEY")
        if api_key:
            return cls(api_key=api_key)

        raise RuntimeError(
            "Google Drive sync needs one of MODDULAR_GRAPH_GOOGLE_SERVICE_ACCOUNT_JSON, "
            "MODDULAR_GRAPH_GOOGLE_DRIVE_ACCESS_TOKEN, or MODDULAR_GRAPH_GOOGLE_DRIVE_API_KEY."
        )
    ...
    def create_file(self, parent_id: str, source: Path, name: str | None = None, mime_type: str | None = None) -> dict[str, Any]:
        boundary = f"moddular-{uuid.uuid4().hex}"
...
def google_drive_sync_enabled() -> bool:
    return bool(os.environ.get("MODDULAR_GRAPH_GOOGLE_DRIVE_FOLDER_ID"))
...
def sync_google_drive_storage(
    *,
    ...
    root_folder_id = root_folder_id or os.environ.get("MODDULAR_GRAPH_GOOGLE_DRIVE_FOLDER_ID", "").strip()
    if not root_folder_id:
        return {"status": "skipped", "reason": "MODDULAR_GRAPH_GOOGLE_DRIVE_FOLDER_ID is not set."}

    ttl = int(os.environ.get("MODDULAR_GRAPH_GOOGLE_DRIVE_SYNC_TTL_SECONDS", str(DEFAULT_TTL_SECONDS)))
    marker = _sync_marker(data_dir)
    ...
    if database and database.is_folder:
        downloaded.extend(
            _download_named_files(client, database.id, data_dir / "01_Database", {"moddular_graph.sqlite3"})
        )
    else:
...
def write_back_mcp_tokens_file(*, data_dir: Path = DATA_DIR, client: GoogleDriveClient | None = None) -> dict[str, Any]:
    return write_back_google_drive_file(
        Path(os.environ.get("MODDULAR_GRAPH_MCP_TOKENS_FILE") or MCP_TOKENS_FILE),
        ["00_Admin"],
        client=client,
...
def _resolve_folder_path(client: GoogleDriveClient, root_folder_id: str, folder_names: list[str]) -> str:
...
def _load_service_account_info() -> dict[str, Any] | None:
    raw_json = os.environ.get("MODDULAR_GRAPH_GOOGLE_SERVICE_ACCOUNT_JSON")
    if raw_json:
        return json.loads(raw_json)

    raw_b64 = os.environ.get("MODDULAR_GRAPH_GOOGLE_SERVICE_ACCOUNT_JSON_B64")
    if raw_b64:
        try:
            return json.loads(base64.b64decode(raw_b64.strip()).decode("utf-8"))
...
def _gcloud_access_token() -> str:
    command = os.environ.get("MODDULAR_GRAPH_GCLOUD_COMMAND") or "gcloud"
    try:
